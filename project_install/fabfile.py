...
def initialize_django_env():
    ...
    with cd('%(base_install)s' % env):
        ...
        with prefix('workon %(projet)s' % env):
            ...
            run('python ./manage.py loaddata coop_local/fixtures/{area_types,django_site,exchange_methods,location_categories,roles,user,legalstatus,organizationcategory,organizationcategoryiae,agreementiae,contact_mediums,relation_types,group}.json')
