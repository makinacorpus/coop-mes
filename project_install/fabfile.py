...
def initialize_django_env():
    ...
    with cd('%(base_install)s' % env):
        ...
        with prefix('workon %(projet)s' % env):
            ...
            run('python ./manage.py loaddata coop_local/fixtures/{areatypes,django_site,exchange_methods,linkproperty,location_categories,top_roles,uriredirect,user,legalstatus,organizationcategory,organizationcategoryiae,agreementiae}.json')
